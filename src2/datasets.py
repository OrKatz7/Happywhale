...
class TrainDataset(Dataset):
    def __init__(self, df, TRAIN_PATH, transform=None,crop_p=None,crop_csv_path=None,crop_backfin_csv_path = None,mode='train',load_all=False,remove_background=False):
        self.df = df
        self.TRAIN_PATH = TRAIN_PATH
        self.file_names = df['image'].values
        self.labels = df['classes'].values
        self.labels_s = df['classes_species'].values
        self.count = df['count_individual_id'].values
...
def GetTrainDataLoader(folds,fold,train_transforms,val_transforms,batch_size,num_workers,data_root_path,crop_p,crop_csv_path,crop_backfin_csv_path,use_crop_for_val,use_sampler=True,epochs=25):
