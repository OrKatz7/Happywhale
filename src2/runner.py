...
def valid_fn(config,model,train_loader,valid_loader,device,run_cv = True,only_softmax=True):
    emb_v,targets_v,y_preds = emb_fn(valid_loader, model, device)
    ### softmax head  
    metric_softmax = [] 
    for row in range(len(y_preds)):
        m = map_per_image(targets_v[row],y_preds[row])
        metric_softmax.append(m)
    if only_softmax:
        return np.array(metric_softmax).mean(),0
