...
class config:
    ...
    singles_in_fold = True
    singles_in_train = True
    train_not_seen=True
    csv_use_new=True
    valid_use_species = True
    min_num_in_train=1
    batch_size = 16
    num_workers = 12
    n_class_skip = 1
    ...
    scheduler = {'f_class': "torch.optim.lr_scheduler.CosineAnnealingWarmRestarts",
             "args": {
                     ...
                     'eta_min':1e-7,
                     'last_epoch':-1}}
    criterion = [{'f_class': "torch.nn.CrossEntropyLoss","args": {'ignore_index':-1},'name':"arcface" , 'w':0.5,"label":"label"},
                {'f_class': "torch.nn.CrossEntropyLoss","args": {'ignore_index':-1},'name':"species", 'w':0.1,"label": "species"},
                {'f_class': "torch.nn.CrossEntropyLoss","args": {'ignore_index':-1},'name':"u_id",'w':0.1,"label":"label"},
                {'f_class': "models.losses.SeenBCEwithLogic","args": {},'name':"not_new",'w':0.3,"label":"label"}, 
