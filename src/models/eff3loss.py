...
class BlockAttentionModel(nn.Module):
    def __init__(
        ...
        self.local_bn = nn.BatchNorm2d(fc_dim)
        self.local_bn.bias.requires_grad_(False)  # no shift
        self.bottleneck_g = nn.BatchNorm1d(self.n_features)
        self.bottleneck_g.bias.requires_grad_(False)  # no shift
        self.local_conv = nn.Conv2d(self.n_features, fc_dim, 1)
    ...
    def forward(self, x):
        ...
        species = self.head2(out)
        out = self.head(out)
        
        return {'out':out,'species':species,"global_feat":global_feat,"local_feat":local_feat}
