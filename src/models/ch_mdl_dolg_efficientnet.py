...
class MultiAtrousModule(nn.Module):
    def __init__(self, in_chans, out_chans, dilations):
        ...
        self.d0 = nn.Conv2d(in_chans, 512, kernel_size=3, dilation=dilations[0],padding=3)
        self.d1 = nn.Conv2d(in_chans, 512, kernel_size=3, dilation=dilations[1],padding=6)
        self.d2 = nn.Conv2d(in_chans, 512, kernel_size=3, dilation=dilations[2],padding=9)
...
class Net(nn.Module):
    def __init__(self, cfg, df):
        ...
        if self.cfg.loss == 'adaptive_arcface':
            # tmp = np.sqrt(1 / np.sqrt(df['classes'].value_counts().sort_index().values))
    ...
    def forward(self, batch):
        ...
        if self.training:
            ...
            return {'loss': loss, "target": batch['target'], "preds_conf":preds_conf,'preds_cls':preds_cls,'logits':logits}

        else:
            loss = torch.zeros((1),device=dev)
            return {'loss': loss, "target": batch['target'],"preds_conf":preds_conf,'preds_cls':preds_cls,
                    'embeddings': x_emb,'logits':logits
                   }
